...
export const createInventoryItem = async (req, res) => {
  ...
  const itemsData = req.body; // Expecting an array of items now

  // Validate the input
  if (!Array.isArray(itemsData) || itemsData.length === 0 || !inventoryId) {
    return res
      .status(400)
      .json({ error: "Missing required item data or inventory ID." });
  }

  try {
    const newInventoryItems = await inventoryService.createInventoryItem(
      itemsData,
      parseInt(inventoryId)
    );
    // Respond with the count of items created, as createMany returns a count
    res.status(201).json({
      message: "Successfully added items to inventory.",
      count: newInventoryItems.count
    });
  } catch (error) {
