...
export const getNewAccessToken = async (req, res) => {
  ...
  try {
    const newAccessToken = await userService.refreshAccessToken(refresh_token)
    res.status(200).json({
      newAccessToken
    })
  } catch (error) {
...
export const createStaffUser = async (req, res) => {
  ...
  try {
    ...
    const newUserData = {
      ...
      role,
      password,
      managerId: currentUser.userId, // Link the new user to the manager/admin
    };
