...
export const addStaffToStore = async (req, res) => {
  ...
  try {
    if (Array.isArray(users)) {
      ...
      const newStoreStaffs = await storeService.addStaffsToStore(users);
      res.status(201).json(newStoreStaffs);
    } else {
      ...
      const staff = await storeService.addStaffToStore(
        parseInt(id),
        userId,
        role
      );
...
export const deleteStoreStaffs = async (req, res) => {
  const { id } = req.params
  const { userIds } = req.body
  console.log(id, userIds)
  if (!id || !userIds || !Array.isArray(userIds) || userIds.length === 0) {
    return res.status(400).json({ error: "Store ID and at least one User ID are required." });
  }
  try {
    const removedStaff = await storeService.removeStaffsFromStore(
      parseInt(id), userIds
    )
     res.status(200).json(removedStaff)
  } catch (error) {
      console.log(error)
      return res.status(500).json({
        error: 'An unexpected error has occured while deleteing staffs'
      })
  }
}
...
/**
 * @description
 * Retrieves list of store staffs by its ID.
 */
export const getStoreStaffs = async (req, res) => {
  const { id } = req.params;

  try {
    const staffs = await storeService.getStoreStaffs(parseInt(id));
    res.status(200).json(staffs);
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      error: "An unexpected error occurred while fetching store staff.",
    });
  }
};
/**
...
/**
 * @description
 * Retreives inventory Items to that store. if the user is assigned to that store
 */
...
export const getStoreItems = async (req, res) => {
  ...
  if (!user) {
    return res.status(403).json({
      error: "Forbidden: No User.",
    });
  }
  try {
    const storeItems = await storeService.getStoreItemsByAssingedStaff(user.userId)
    res.status(200).json(storeItems)
  } catch (error) {
    console.error("Error while getting store items: ",error)
    return res.status(500).json({
      error: "An internal server error has occured"
    })
  }
