...
export const createInventoryItem = async (req, res) => {
  const { id } = req.params; // Get the inventoryId from the URL
  let itemData = req.body; // Separate item and location data from the body
  //if (!locationData) {
  //  locationData = null
  // }
  if (!itemData || !id) {
    return res
      .status(400)
      .json({ error: "Missing required item, or inventory data." });
  }
  console.log(itemData);
  console.log("Invntory Item", id);
