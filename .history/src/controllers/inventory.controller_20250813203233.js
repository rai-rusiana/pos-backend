...
export const updateInventory = async (req, res) => {
  const id = req.params;
  const inventoryData = req.body; // Correctly get the entire body
...
export const createInventoryItem = async (req, res) => {
  ...
  const { itemData, locationData } = req.body; // Separate item and location data from the body

  if (!itemData || !locationData || !inventoryId) {
    return res
      .status(400)
...
export const getInventoryItemsByRack = async (req, res ) => {
     ...
     try {
          const inventory = await inventoryService.getInventoryItemsByRack(parseInt(id), rackName)
          if (!inventory) {
               return res.status(404).json({
                    error: 'Inventory not found'
               })
